...
- 사용자 제공 `위원명단.xlsx`에서 추출한 `data/committee-news/roster.json`(산자위 24명·기재위 22명·정무위 24명)을 의원 분류 기준으로 사용한다. 의원 발언에는 `members`와 소속 검토 근거를 기록한다. 이름만으로 분류하지 않는다. 장관·후보자 자격의 답변은 소속 의원 상임위가 아닌 소관·심사 상임위로 분류한다.
- 동일 사안·동일 원문은 한 시간대에서 3개 상임위 중 하나에만 배정한다. `committees`는 반드시 1개이며 주된 발언 의원, 심사 위원회, 소관 정책 순으로 실제 맥락을 검토한다. 유류세·재정은 기재위, 공정거래·담합은 정무위, 산업정책·통상은 산자위로 구분하되 기사 근거를 우선한다.
