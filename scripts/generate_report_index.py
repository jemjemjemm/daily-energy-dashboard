...
def render_chart(series: Mapping[str, Sequence[Mapping[str, Any]]], colors: Mapping[str, str]) -> str:
    ...
    # Hover targets only: no visible dots. Keep the original clean line-chart design.
    # Use data-tooltip because native SVG <title> tooltip is unreliable inside iframe/mobile browsers.
    for name, pts in series.items():
        for p in pts:
            d = str(p.get("date"))
            if d not in date_idx:
            ...
            parts.append(
                ...
                f'class="hover-hit" data-tooltip="{tooltip}" />'
            )
...
def css() -> str:
  ...
  * { box-sizing:border-box; }
  ...
  .chart-tooltip { position:fixed; z-index:9999; pointer-events:none; background:rgba(10,36,68,.94); color:#fff; border-radius:8px; padding:6px 8px; font-size:11px; line-height:1.35; box-shadow:0 6px 18px rgba(0,0,0,.18); transform:translate(10px, -28px); white-space:nowrap; }
...
(function () {
  ...
  function move(evt) {
    ...
    if (x > maxX) x = Math.max(8, evt.clientX - tooltip.offsetWidth - 12);
  ...
  document.addEventListener('mouseover', function (evt) {
    if (evt.target && evt.target.classList && evt.target.classList.contains('hover-hit')) {
      show(evt);
    }
  });
  ...
  document.addEventListener('touchstart', function (evt) {
    var target = evt.target;
    if (target && target.classList && target.classList.contains('hover-hit')) {
      show(evt.touches[0] || evt);
