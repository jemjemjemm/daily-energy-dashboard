...
오피넷 국제유가 JSON과 장기 history.json을 리포트용 JSON 초안에 반영하는 스크립트 v1.2

핵심 정책
- data/prices/history.json에는 2026년 이후 장기 가격 데이터를 계속 보유한다.
- 리포트 화면의 그래프에는 기준일 전일 기준 과거 2개월 구간만 표시한다.
  예: --date 2026-05-20
      그래프 표시 기간: 2026-03-19 ~ 2026-05-19
- 값이 0인 가격은 그래프에서 제외한다.
- 가격 카드: 당일 수집 JSON의 최신 가격 기준
- 가격 추이 그래프: history.json 우선 사용, 없으면 당일 수집 JSON의 chart_series 사용
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--chart-months", type=int, default=2, help="그래프에 표시할 과거 개월 수. 기본값 2개월")
...
def clean_series(series: Mapping[str, List[Mapping[str, Any]]], start_date: str, end_date: str) -> Dict[str, List[Dict[str, Any]]]:
...
def choose_chart_series(price_data: Mapping[str, Any], history: Optional[Mapping[str, Any]], start_date: str, end_date: str):
    crude_history_series = build_series_from_history(
        history, "crude", {"Brent": "Brent", "WTI": "WTI", "Dubai": "Dubai"}, start_date, end_date
    )
    product_history_series = build_series_from_history(
        history, "products", {"Gasoline": "Gasoline_92RON", "Diesel": "Diesel_0.001", "Naphtha": "Naphtha"}, start_date, end_date
    )
    ...
    if sum(len(points) for points in crude_history_series.values()) > 0:
        ...
        crude_series = clean_series(price_data.get("crude", {}).get("chart_series", {}) or {}, start_date, end_date)
    ...
    if sum(len(points) for points in product_history_series.values()) > 0:
        ...
        product_series = clean_series(price_data.get("products", {}).get("chart_series", {}) or {}, start_date, end_date)
...
def update_price_section(report: Dict[str, Any], price_data: Mapping[str, Any], history: Optional[Mapping[str, Any]],
    ...
    report["prices"] = {
        ...
        "price_data_note": (
            ...
            f"가격 추이 그래프는 기준일 전일 기준 과거 2개월({short_date(start_date)}~{short_date(end_date)})만 표시하며, "
            "값이 0인 가격은 제외했습니다."
        ),
        "crude": {
...
def update_sources_and_quality(report: Dict[str, Any], price_data: Mapping[str, Any], history: Optional[Mapping[str, Any]],
    ...
    sources = [source for source in sources if not (source.get("type") == "price" and "오피넷" in source.get("name", ""))]
    sources.extend([
...
def update_automation(report: Dict[str, Any], price_data: Mapping[str, Any], history: Optional[Mapping[str, Any]],
                      chart_sources: Mapping[str, str], start_date: str, end_date: str) -> None:
    ...
    report["automation"]["opinet_prices"] = {
        ...
        "merge_script": "merge_prices_into_report.py v1.2",
        "needs_review": True,
    }


def merge_prices(report: Dict[str, Any], price_data: Mapping[str, Any], history: Optional[Mapping[str, Any]],
                 report_date: str, months: int) -> Dict[str, Any]:
...
def main() -> int:
    ...
    try:
        ...
        print(f"[OK] 그래프 표시 기간: {short_date(merged['prices']['crude']['chart_window']['start'])}~{short_date(merged['prices']['crude']['chart_window']['end'])}")
